...
<SendScreen>:
    ...
    BoxLayout:
        ...
        CButton:
            text: '📂 CHOOSE FILE'
            background_color: utils.get_color_from_hex('#334155')
            on_release: app.open_native_file_picker()
...
class KShareApp(App):
    ...
    # --- NATIVE FILE PICKER ---
    def open_native_file_picker(self):
        filechooser.open_file(on_selection=self._on_file_selection)

    def _on_file_selection(self, selection):
        if selection:
            self.selected_filepath = selection[0]
            self.root.get_screen('send').ids.selected_file_lbl.text = os.path.basename(self.selected_filepath)
    ...
    def open_dir_chooser(self): filechooser.choose_dir(on_selection=self._on_dir_selection)
    def _on_dir_selection(self, selection):
        if selection:
            self.store.put('config', save_path=selection[0])
    ...
    def close_scanner(self):
        c = self.root.get_screen('scanner').ids.camera_container
        if c.children:
            cam = c.children[0]; cam.play = False; c.remove_widget(cam)
    ...
    def prepare_receiver_ui(self):
        ...
        s.ids.rx_pct_lbl.text = "0%"; s.ids.rx_progress.value = 0; s.ids.rx_status.text = "Waiting..."
        self.start_server()
    ...
    def server_loop(self):
        ...
        try:
            ...
            while self.server_running:
                try:
                    c, addr = sock.accept()
                    Clock.schedule_once(lambda dt: setattr(s.ids.rx_status, 'text', f"Receiving..."))
    ...
    def start_transfer(self):
        s = self.root.get_screen('send'); ip = s.ids.target_ip.text.strip()
        if not self.selected_filepath or not ip: return
        threading.Thread(target=self.send_file_thread, args=(self.selected_filepath, ip)).start()
    ...
    def send_file_thread(self, path, ip):
        ...
        try:
            ...
            sock.close()
            Clock.schedule_once(lambda dt: setattr(s.ids.status_lbl, 'text', "Done!"))
            Clock.schedule_once(lambda dt: setattr(s.ids.pct_lbl, 'text', "100%"))
        except: Clock.schedule_once(lambda dt: setattr(s.ids.status_lbl, 'text', "Failed!"))
